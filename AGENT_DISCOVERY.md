...
For script-aligned interview editing, use `script-clip-candidate-pack <bundle> <request.json>` / MCP `script_clip_candidate_pack` to orchestrate the existing VideoRAG, moment-index and transcript-reference-window owners into a local candidate pack. After human selection and fine cutting, use `script-clip-alignment-check <bundle> <review-notes.json> <fine-cut-plan.json>` / MCP `script_clip_alignment_check` to compare the approved source quote, keep ranges, anonymous speakers, clip-only ASR and subtitle. Both contracts are review-only and always set `publication_allowed=false`; missing clip ASR, unresolved or excluded speakers, sentence fragments, subtitle expansion and source/clip disagreement fail closed. See `docs/script-clip-alignment-workflow-2026-08-16.md`.

For precise extraction of arbitrary video content, use `content-clip-candidate-pack <bundle> <request.json>` / MCP `content_clip_candidate_pack`. The versioned profiles are `spoken-quote-v1`, `lecture-explanation-v1`, `tutorial-step-v1`, `visual-event-v1`, `screen-content-v1`, `audio-event-v1`, `broll-v1`, `highlight-v1`, and `story-beat-v1`. They reuse the same VideoRAG/moment/transcript owners and add only existing OCR/temporal/technical-shot coverage plus content-specific recommended/safe boundaries. After human selection and cutting, use `content-clip-alignment-check` / MCP `content_clip_alignment_check` with hash-bound clip-only ASR/OCR/visual/audio evidence. Missing required modalities, stale hashes, unsafe boundaries, speaker conflicts, semantic expansion, or missing technical shots fail closed. No command executes models, cuts media, uploads, infers identity, approves, or publishes. See `docs/content-clip-retrieval-and-alignment-v1-2026-08-16.md`.

For very long videos, use `long-video-fast-segment-plan <bundle>` / MCP `long_video_fast_segment_plan` to combine the existing timestamped transcript, VAD, Timeline/OCR, and verified technical shots into review-only removal candidates. `apply-long-video-fast-segment-review` requires an exact keep/drop decision for every candidate and current input hashes; it still does not edit media. `render-long-video-fast-segment` is preview-only unless `--execute` is explicit, never overwrites the source, uses the shared FFmpeg resolver/receipt, and writes duration QA. Low-confidence or conflicting evidence defaults to keep; no provider call, upload, silent fallback, or publish occurs. See `docs/long-video-fast-segmentation-2026-08-18.md`.
